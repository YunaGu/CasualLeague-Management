...
2. 在【云开发 → 数据库】中新建集合 `tournaments` 和 `admins`。
3. 将 `admins` 权限设置为【仅管理端可读写】。每个管理员添加一条文档，文档 `_id` 必须是该用户的 OpenID，并设置 `enabled: true`。
4. 将 `tournaments` 切换为自定义安全规则：

```json
{
  "read": true,
  "write": "auth != null && get(`database.admins.${auth.openid}`).enabled == true"
}
```

5. 在微信开发者工具中右键 `cloudfunctions/wxContext`，选择【上传并部署：云端安装依赖】。
6. 如果项目有多个云环境，在 `miniprogram/config.js` 的 `cloudEnvId` 中填写环境 ID；留空时使用当前小程序的默认环境。
7. 重新编译小程序。非管理员首页会显示只读模式和“复制我的管理员配置 ID”，将复制的 OpenID 添加到 `admins` 集合后，重新打开小程序即可获得管理权限。
